    ...
    if querystring['view'] == 'fixes' or querystring['view'] == 'affected':
        ...
        if querystring['version'] in versions:
            ...
            if querystring['issuekey'] in version:
                ...
                data = {
                    'resolution': issueMap.resolution,
                    'issuetype': issueMap.issuetype,
                    'priority': issueMap.priority,
                    'assignee': issueMap.assignee,
                    'reporter': issueMap.reporter,
                    'affectedVersions': issueMap.reporter
                }
                ...
                data['issuetype'] = le.fit_transform(data['issuetype'])
                data['priority'] = le.fit_transform(data['priority'])
                data['assignee'] = le.fit_transform(data['assignee'])
                data['affectedVersions'] = le.fit_transform(data['affectedVersions'])
                data['reporter'] = le.fit_transform(data['reporter'])

                values = df[['resolution', 'issuetype', 'priority', 'assignee', 'reporter']]
                target = df['affectedVersions']

                #Prepear training and testing data
